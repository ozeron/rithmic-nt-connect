...
| KTD4 | Dedup key = `trade_id_from_fill_fields(...)` optionally prefixed with account + instrument; set on exec client; clear on disconnect; insert only after successful parse/route | Reuses existing helper; conventions require extra dimensions when venue id is weak |
