...
| KTD4 | Dedup only when venue `fill_id` present (`acct|inst|fill_id`); retain across disconnect (bounded LRU); mark only after successful emit. Without `fill_id`, never suppress (prefer duplicate over dropped fill) | Reuses existing helper; weak synthetic keys must not collapse distinct fills |
