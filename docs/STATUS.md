...
**History ticks / time bars.** Rust slices (15m ticks / 4h bars), transient + empty retry, sort/dedup. Daily/weekly replay uses calendar `YYYYMMDD` indexes (Lucid 2026-08-13). Python: `load_front_month_instrument` / `load_trade_ticks` / `load_time_bars`. **RC4 (2026-08-31):** `plants connected` ≠ history Ready — `RithmicSession::connect` proves a tiny 1m Load* before Ready; `request timed out` is transient (1–2 retries). RC2.3 refuse-under-live-MD unchanged; qgw fail-closes empty mid-session lookback.
...
2. **Execution honesty** — [~] three evidence classes; untracked → reports; recon is a bounded `show_orders` drain (best-effort, not provably complete); dedup by venue id. `cancel_all_orders` is N/A for this line's honesty claim (2026-08-22): plant-wide panic only (`RITHMIC_GATEWAY_CANCEL_ALL`), not advertised OM (TC-E41 skip).
