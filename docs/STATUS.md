...
| Reconnect + MD resubscribe | Planned | **Partial** — ticker poll resyncs last-trade/BBO + book + EXTERNAL bar intent. Gateway typed restore covers ticker/book/bars/PnL/order/brackets after plant drop. Data resync is now scoped: direct resets **only** the ticker plant (`reset_ticker_plant`; PnL/order untouched), gateway re-dials only that client and re-issues ticker/book/bar intents via `ResetTickerPlantRequest` (wired both paths, framing-tested). Not Lucid-proven. |
