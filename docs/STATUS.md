...
> Regenerate this table with `python scripts/status_progress.py` and keep it in sync
> (`python scripts/status_progress.py --check` fails CI/docs if it drifts from the marks).
> "Implemented %" counts done items plus half of partial items. `N/A` rows are out of
> scope and are not counted as "not started".
...
Not started (the real "left to do"): `3` marks — Phase 2 definition updates, Phase 7 recovery suite, and one sub-item.
...
| Axis | Planned | Status |
| --- | --- | --- |
| Products | Futures only | [x] for futures (`FuturesContract` from reference + front month). Other families **N/A**. No live definition-update stream. |
| Environments | `SessionConfig.env` Live / Demo / Test | [x]. LucidTrading + `wss://rprotocol.rithmic.com:443`. |
| Account modes | Margin + `OmsType.NETTING`; trading gated | [~]. `enable_trading=False` default. Trading / PnL still require FCM / IB / account triple. |
| Live trades / quotes | `TradeTick`, `QuoteTick` | [x] |
| Order-book summary | `OrderBookDeltas` (L2) | [~] — snapshot flags `F_SNAPSHOT` / `F_LAST`. Book unsubscribe wired through plants/gateway/direct. |
| History ticks / time bars | Request path, `*_all` | [x] — Rust slices (15m ticks / 4h bars), transient + empty retry, sort/dedup. Daily/weekly replay uses calendar `YYYYMMDD` indexes (Lucid 2026-08-13). Python `load_front_month_instrument` / `load_trade_ticks` / `load_time_bars`. |
| Live venue `EXTERNAL` bars | 1m / 15m / 1h / 1d | [~] — history `request_bars` + history-plant `subscribe_time_bar_updates`. 1m EXTERNAL subscribe **live-proven on LucidTrading 2026-08-14** (`test_TC_D40_subscribe_external_bars`). 1s stays INTERNAL. Node-path dispatch fixed 2026-08-19: the venue echoes the `time_bar` event `period` in seconds (`"60"`) while the dispatch key was registered with the native period (`1`), so node-path EXTERNAL bars were dropped as `time_bar_unsubscribed` — keys are now registered under both units (unit-tested). The INTERNAL-vs-EXTERNAL seam is pinned deterministically by `tests/test_bar_seam_parity.py`; the live full-node parity e2e (TC-D54) was dropped because the Rithmic Test ticker plant goes silent (no incremental trades/BBO, stale synthetic snapshots) while its bar plant runs, so a live INTERNAL==EXTERNAL proof is not reliably hostable on Test — live bar delivery remains proven by TC-D40. |
...
| Brackets / OCO | Plant bracket API | [~] — wire on direct + gateway; Lucid accept/survival **not proven**. Live spike: `scripts/spike_bracket_order.py` (`RITHMIC_BRACKETS=1` + `RITHMIC_ENABLE_TRADING=1`; spike-only flag, not enforced in plants/gateway). |
| Account / positions | Best-effort PnL | [~] — auto-discovers FCM/IB/account when unset (multi-account needs `RITHMIC_ACCOUNT_ID` selector); soft-fail PnL otherwise |
| Submit / cancel / modify + fills | Gated order plant | [~] — submit pre-send deny; post-send unknown; untracked → reports; fill dedup. 2026-08-19 exec-hardening: overfill emits + latches (never-drop/cap); stale/duplicate TRIGGER suppressed; basket-path `is_closed` guard; venue sentinel `-1.0` → `None`; in-flight query fails closed + per-`user_tag` drain recovery (strict usable-row validation); reconnect re-arms only after a bounded drain **and** a fresh PnL/position observation (activity gate; skipped when the PnL stream soft-fails — the plant then re-arms on the drain alone with blind position context, see ops-runbook); the tag drain binds only usable rows and never regresses a live-resolved order; a dead/broken order stream (handler break / resync failure) latches or flags so the re-arm barrier cannot clear over it; unpriceable fills suppress without consuming the dedup key and latch; the plant lifecycle is an explicit state machine (`LATCHED` is a real state, `rearm` the only arming transition, a down plant cannot re-arm via resync, a resync during the barrier latches — transition table pinned by `tests/test_order_plant.py`) and the drain-row interpretation is one boundary (`_drain_row_from_fields`; build-then-bind, publish-before-bind with the barrier aborting on publish failure). Oracle 4th pass: strict recovery recognizes `triggered` rows (TRIGGERED for limit-style stops), modify/cancel-rejected report as working (never terminal REJECTED), per-tag recovery answers with the newest row, and persistent transient order-stream errors latch — the streak is loop-local (per stream lifetime: a fresh poll loop or successful resubscribe starts at zero, so old counts can never latch a healthy loop); the re-arm drain skips closed tracked orders (no duplicate external status) and malformed `ts_event`/boolean closed-set rows never abort or bind. Root-cause refactor (Macroscope round 3): the transient streak is loop-local state (no client attribute to carry across lifetimes), the raw-row drain pipeline is ONE owned iterator (`_iter_drain_rows` — normalize/basket/ts guards inside, four loops consume it), drain identity is a dedicated `_drain_client_order_id` (cache-first, closed-order guards apply), and a mutation-corpus property test makes "bindable ⟹ real closed-set terms" a checked class. Root-cause refactor (round 3, coercion-as-validation): `price_type`/`duration` are normalized to an exact int or `None` at the convert boundary (`enum_int` — bools/non-integral numerics never survive), `_row_is_trustworthy` collapses to the same whitelist (presence + known-value only), the report builders own the event-time timestamp fallback (no consumer can publish an epoch-0 status/fill), and convert-boundary property tests pin the whitelist (Oracle 2nd pass + root-cause refactor 2026-08-19). Macroscope round 4 (2026-08-19): a timestamp-less drain row is never skipped by the freshness rule (0 is the synthetic missing-ts value — the snapshot still publishes and binds), the in-flight query gate uses the effective venue-id lookup (an order whose venue id lives on the model answers directly, no drain), and a venue fill without `fill_id`/timestamp dedupes to a stable `TradeId` (clock fallback only in the report timestamp, never the identity). Commission (2026-08-19): fills now carry venue RMS commission — per-product `commission_fill_rate` (e.g. MNQ 0.5) x fill qty with the account `default_commission` as fallback, fetched at connect on both paths (direct PyO3 + gateway RPC `fetch_product_rms_info` / `fetch_account_rms_info`, gated on parent trading like `load_orders`); fetch failure is non-fatal (zero fallback). Live rates are verified per account by the mechanism-only e2e test `tests/e2e/test_rms_commission_live.py` (direct + gateway round-trip, no live-value expectations). Test-plant live place confirmed 2026-08-17 (default `app_name` authorized to exchange). |
| Order status reports | Venue recon when trading; cache-backed when read-only | [~] (best-effort) — `load_orders` drains the current working orders via `show_orders` + a bounded silence window. Not provably complete (no end-of-list signal; replays cap at 10k), so the result is advisory, not an authoritative venue snapshot. Read-only returns cache-backed. |
| Fill reports | Venue recon when trading; read-only declines | [~] (best-effort) — `generate_fill_reports` filters the same bounded `load_orders` drain for filled rows; working-order replays carry no historical fills, so fills are only those seen on the live stream. Read-only declines. |
...
| Reconnect + MD resubscribe | Planned | [~] — every path that re-establishes the wire re-issues ticker + book + EXTERNAL bar intent through one replay boundary (`replay_subscription_intent`): the channel-error resync (`_resync_ticker_subscription` → `reset_ticker` + replay) **and** the client lifecycle (`_connect` re-issues remembered intent, clears one-sided BBO accumulators, restarts the history poll when bars are registered — a disconnect→connect previously left live plants with zero subscriptions). Gateway typed restore covers ticker/book/bars/PnL/order/brackets after plant drop. Data resync is scoped: direct resets **only** the ticker plant (`reset_ticker_plant`; PnL/order untouched), gateway re-dials only that client via `ResetTickerPlantRequest` (wired both paths, framing-tested). Not Lucid-proven. |
| Session gateway (shared login) | `connect_mode` required (`direct` / `gateway`) | [~] — plants/session parity on wire (incl. book unsub, `probe_time_bars`, ensure_order, `resolved_account`). Direct is a process-wide singleton per credential fingerprint (data factory, exec factory, and `connect_market_data_session` share one login; flock taken once); the credential flock is released when the **last** holder disconnects, so a stopped node no longer blocks a separate process. `session_pb2.py` regenerated reproducibly with uv-pinned protoc (`grpcio-tools==1.71.0` bundles 29.0; `scripts/gen_gateway_proto.py` restores the 5.29.6 marker; byte-identical output). Auto-spawn idle-exit after last client (`RITHMIC_GATEWAY_IDLE_EXIT_SEC`, default 5s via spawn; unset = never for standalone). Shared-consumer Lucid smoke green. Native TLS remote **N/A**. **Wheel:** self-contained — `scripts/build_wheel.sh` bundles the `rithmic-gateway` binary + `rithmic_gateway` client into the maturin wheel; `resolve_gateway_bin` prefers `rithmic_gateway/bin/`. **Lake:** market-data-lake hardcodes `GatewayClient` + `load_time_bars_range` (client-side chunks); install `python/` package (no Nautilus). |
...
6. **Exec hardening (e2e TDD)** — [~] 2026-08-19 P1 (in-flight fail-closed + per-tag drain + drain-gated re-arm), P2 (A2 overfill, A1 stale trigger, A5 basket guard, A3 sentinel), P3 (race/recovery), P4 (gateway-facade restore) landed with red→green transport-e2e tests. Oracle 2nd-pass fixes landed: latch-generation guard, PnL-snapshot re-arm gate, validate-then-bind + stale-row re-read in the tag drain, venue-row `ts_event` in recovered reports. Oracle merge items landed: stream-failure latch, strict report-builder validation, PnL-marker placement + activity-gate doc, unpriceable-fill latch. Oracle simplification pass landed: latch-generation counter + stream-failed flag collapsed into plant state (re-arm keyed on `CONNECTING` + poll-task liveness), unresolved in-flight queries always fail closed (no `None`), per-tag drain cooldown removed, `-1.0 → None` centralized at the convert boundary (fill classification decoupled from priceability), PnL gate is an `asyncio.Event`, and the drain retry loop dropped to one bounded attempt. Root-cause refactor (Macroscope round 2 + Oracle): the plant lifecycle is now an explicit state machine (`OrderPlantPolicy` — `LATCHED` a real state, owned transitions, `rearm` the only arming transition, a `DISCONNECTED` plant cannot re-arm via resync, a mid-barrier resync latches), the drain-row interpretation is one boundary (`_drain_row_from_fields`, build-then-bind + publish-before-bind in every drain path), and the transition table + row-boundary property tests make the invariants a checked class (`tests/test_order_plant.py`). Oracle 4th pass (2026-08-19): `triggered` strict rows bind and report TRIGGERED for limit-style stops; modify/cancel-rejected report as working (never terminal REJECTED); per-tag recovery picks the newest matching row; drain publish failure aborts the re-arm barrier; stale drain rows skip when the live stream advanced the order; persistent transient order-stream errors latch; `rearm` requires a real poll task. Root-cause refactor round 2 (Macroscope round 3): the transient streak became loop-local state (deleted the client attribute — a fresh loop/resubscribe starts at zero structurally), the raw-row drain pipeline collapsed into one owned iterator (`_iter_drain_rows`, all normalize/basket/ts guards inside; the four drain loops are consumers), drain identity is a dedicated `_drain_client_order_id`, and a mutation-corpus property test pins "bindable ⟹ real closed-set terms". A4 (client-order-id validation) parked on OQ1 (Rithmic `user_tag` constraint unsourced). P5 live canary on the test account not yet run. Live data e2e re-verified 2026-08-19 (15 passed on Rithmic Test) after fixing two pre-existing `main` regressions from the 8862b62 tooling commit: the flocked direct session forwarded a `timeout_ms` arg the no-arg PyO3 `poll_event` rejects, and the `LiveDataClient` dual-base MRO broke `RithmicDataClient.__init__` (`TradingNode.build` raised missing-`loop`); the single base is restored and the `LiveDataClient` factory contract is satisfied by the vendored stub.
